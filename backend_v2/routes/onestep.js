...
router.post("/ordini/:id/ricevi", validate({ params: idParam, body: riceviSchema }), (req, res) => {
  try {
    ...
    logger.error({ err }, "/onestep/ordini/:id/ricevi");
    res.status(500).json({ error: err.message });
  }
});
...
router.post("/reset", validate({ body: resetSchema }), (req, res) => {
  try {
    ...
    logger.info({ stats }, "Reset One Step (ordini+movimenti)");
    res.json({ ok: true, eliminati: stats });
  } catch (err) {
    logger.error({ err }, "/reset One Step");
    res.status(500).json({ error: err.message });
  }
});
  ...
  try {
    ...
    for (const { asin } of prodotti) {
      try {
        ...
        logger.warn({ asin, status: e.response?.status, err: e }, "[OneStep sync-images] errore ASIN");
        errori.push(asin);
    ...
    logger.error({ err }, "sync-images One Step");
    res.status(500).json({ error: err.message });
  }
});
